...
export function OrganizationFormDialog({ open, customer, onboarding, onClose, onSaved }: Props) {
  ...
  return <><div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4" role="presentation"><div role="dialog" aria-modal="true" aria-labelledby="organization-form-title" className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-3"><div><h2 id="organization-form-title" className="text-xl font-bold">{editing ? 'Editar organização' : 'Novo cliente municipal'}</h2><p className="mt-1 text-sm text-slate-500">Cadastro, política de sessão e marcos de implantação.</p></div><button ref={closeRef} onClick={onClose} aria-label="Fechar formulário" className="rounded-lg p-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"><X className="h-5 w-5" /></button></div><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><Field label="Nome de exibição" value={draft.displayName} onChange={(value) => update('displayName', value)} required />{!editing && <Field label="Identificador" value={draft.slug} onChange={(value) => update('slug', slugify(value))} required />}<Field label="Razão social" value={draft.legalName} onChange={(value) => update('legalName', value)} /><Field label="Município" value={draft.municipalityName} onChange={(value) => update('municipalityName', value)} /><Field label="UF" value={draft.stateCode} onChange={(value) => update('stateCode', value.toUpperCase().slice(0, 2))} /><SelectField label="Status" value={draft.status} values={['onboarding', 'pilot', 'active', 'suspended', 'archived']} onChange={(value) => update('status', value)} /><Field label="Contato" value={draft.contactName} onChange={(value) => update('contactName', value)} /><Field label="E-mail" type="email" value={draft.contactEmail} onChange={(value) => update('contactEmail', value)} /><Field label="Referência contratual" value={draft.contractReference} onChange={(value) => update('contractReference', value)} /><SelectField label="Política de sessão" value={draft.sessionPolicy} values={['block', 'replace']} onChange={(value) => update('sessionPolicy', value)} /><Field label="Timeout da sessão (min)" type="number" value={draft.sessionTimeoutMinutes} onChange={(value) => update('sessionTimeoutMinutes', value)} /><Field label="Tolerância offline (min)" type="number" value={draft.offlineToleranceMinutes} onChange={(value) => update('offlineToleranceMinutes', value)} /><Field label="Início do piloto" type="datetime-local" value={draft.pilotStartedAt} onChange={(value) => update('pilotStartedAt', value)} /><Field label="Coordenação treinada" type="datetime-local" value={draft.coordinatorTrainedAt} onChange={(value) => update('coordinatorTrainedAt', value)} /><Field label="Revisão prevista" type="datetime-local" value={draft.reviewDueAt} onChange={(value) => update('reviewDueAt', value)} /><Field label="Revisão concluída" type="datetime-local" value={draft.reviewCompletedAt} onChange={(value) => update('reviewCompletedAt', value)} /></div>{error && <p className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700" role="alert">{error}</p>}<div className="mt-6 flex justify-end gap-2"><button onClick={onClose} className="rounded-lg border px-4 py-2 text-sm font-semibold">Cancelar</button><button onClick={requestSave} className="flex items-center gap-2 rounded-lg bg-blue-700 px-4 py-2 text-sm font-bold text-white">{mutation.isPending && <Loader2 className="h-4 w-4 animate-spin" />}{editing ? 'Salvar alterações' : 'Criar cliente'}</button></div></div></div><HighRiskDialog open={confirming} title={editing ? 'Confirmar alteração do cliente' : 'Confirmar criação do cliente'} description="A operação exige MFA, justificativa e será registrada na auditoria." confirmLabel={editing ? 'Salvar cliente' : 'Criar cliente'} onClose={() => setConfirming(false)} onConfirm={async (reason) => { const result = await mutation.mutateAsync({ organizationId: customer?.subject_id ?? null, action: editing ? 'update' : 'create', payload, reason }); if (!result.ok || !result.data) throw new Error(result.error || 'Não foi possível salvar o cliente.'); setConfirming(false); onSaved(result.data.customer_id); }} /></>;
}

function Field({ label, value, onChange, type = 'text', required }: { label: string; value: string; onChange: (value: string) => void; type?: string; required?: boolean }) { return <label className="text-sm"><span className="font-semibold text-slate-700">{label}{required ? ' *' : ''}</span><input type={type} value={value} onChange={(event) => onChange(event.target.value)} required={required} className="mt-1 h-10 w-full rounded-lg border px-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600" /></label>; }
