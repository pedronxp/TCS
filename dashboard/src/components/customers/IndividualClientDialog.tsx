...
export function IndividualClientDialog({ open, onClose, onSaved }: Props) {
  ...
  return <>
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4" role="presentation">
      <div role="dialog" aria-modal="true" aria-labelledby="individual-client-title" className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 id="individual-client-title" className="text-xl font-bold">Novo cliente individual</h2>
            <p className="mt-1 text-sm text-slate-500">Crie a conta por convite de e-mail ou defina uma senha inicial.</p>
          </div>
          <button onClick={close} aria-label="Fechar formulário" className="rounded-lg p-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"><X className="h-5 w-5" /></button>
        </div>
        ...
        {mode === 'initial_password' && <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <p className="sm:col-span-2 text-xs text-slate-500">Mínimo de 8 caracteres, com pelo menos uma letra e um número. Envie a senha ao cliente por um canal seguro.</p>
        </div>}

        {mode === 'email_invite' && <div className="mt-5 flex gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"><Mail className="mt-0.5 h-5 w-5 shrink-0" /><p>O link de autenticação não usa nem altera os tokens de cadastro ou os convites de organização do sistema.</p></div>}
        {error && <p className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700" role="alert">{error}</p>}

        <div className="mt-6 flex justify-end gap-2">
          <button onClick={close} className="rounded-lg border px-4 py-2 text-sm font-semibold">Cancelar</button>
          <button onClick={requestProvisioning} className="rounded-lg bg-blue-700 px-4 py-2 text-sm font-bold text-white">{mode === 'email_invite' ? 'Revisar e enviar convite' : 'Revisar e criar conta'}</button>
        </div>
      </div>
    </div>
    <HighRiskDialog
...
function ModeCard({ checked, title, description, onSelect }: { checked: boolean; title: string; description: string; onSelect: () => void }) {
  return <label className={`cursor-pointer rounded-xl border p-4 ${checked ? 'border-blue-600 bg-blue-50 ring-1 ring-blue-600' : 'bg-white'}`}>
    <span className="flex items-start gap-3"><input type="radio" checked={checked} onChange={onSelect} className="mt-1" /><span><b className="block text-sm">{title}</b><span className="mt-1 block text-xs leading-5 text-slate-500">{description}</span></span></span>
  </label>;
}
...
function Field({ label, type = 'text', value, onChange, autoComplete }: { label: string; type?: string; value: string; onChange: (value: string) => void; autoComplete?: string }) {
  return <label className="text-sm"><span className="font-semibold">{label}</span><input required type={type} value={value} onChange={(event) => onChange(event.target.value)} autoComplete={autoComplete} className="mt-1 h-10 w-full rounded-lg border px-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600" /></label>;
}
...
function PasswordField({ label, value, show, onChange, onToggle }: { label: string; value: string; show: boolean; onChange: (value: string) => void; onToggle?: () => void }) {
  return <label className="text-sm"><span className="font-semibold">{label}</span><span className="relative mt-1 block"><input type={show ? 'text' : 'password'} value={value} onChange={(event) => onChange(event.target.value)} autoComplete="new-password" className="h-10 w-full rounded-lg border px-3 pr-10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600" />{onToggle && <button type="button" onClick={onToggle} aria-label={show ? 'Ocultar senha' : 'Mostrar senha'} className="absolute right-2 top-2 rounded p-1">{show ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>}</span></label>;
}
