...
const alertVariants = cva('relative w-full rounded-md border p-4 [&>svg~*]:pl-7 [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground', { variants: { variant: { default: 'bg-background text-foreground', destructive: 'border-destructive/40 bg-status-danger text-destructive [&>svg]:text-destructive', warning: 'border-warning/40 bg-status-warning text-warning-foreground', success: 'border-success/35 bg-status-success text-success' } }, defaultVariants: { variant: 'default' } });
