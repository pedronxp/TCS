...
export function OwnerPage({ title, description, loading, error, actions, children }: { title: string; description: string; loading?: boolean; error?: string | null; actions?: ReactNode; children: ReactNode }) {
  return <div>
    ...
    {loading ? <div className="py-16 grid place-items-center"><Loader2 className="w-7 h-7 animate-spin text-primary" /></div> : error ? <div className="rounded-lg border border-destructive/30 bg-destructive-soft p-4 text-sm text-destructive">{error}</div> : children}
  </div>;
}
