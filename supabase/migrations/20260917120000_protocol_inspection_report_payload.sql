-- Expõe o snapshot de cálculo de risco e as URLs das fotos da vistoria para
-- o Console interno. Necessário para o relatório web gerar o mesmo documento
-- oficial produzido pelo app (utils/laudoPdfBuilder). Os campos continuam
-- restritos a quem possui acesso sensível ao cliente (can_access_sensitive_customer).
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'risk_snapshot', CASE WHEN v_can_sensitive THEN v."calculoRisco" END,
    'photo_urls', CASE WHEN v_can_sensitive THEN
      (SELECT coalesce(jsonb_agg(u), '[]'::jsonb) FROM (
        SELECT v."fotoUrl" AS u WHERE v."fotoUrl" IS NOT NULL
        UNION ALL
        SELECT unnest(coalesce(v."fotosUrls", '{}'::text[]))
      ) todas)
    END
  );
END;
