-- Detalhe de operação: admins/participantes por chat e rastro completo do envio.
-- O bot passa a contar administradores e membros de cada grupo (visão de quem
-- pode manter a comunidade viva se um número cair), e a trilha de tentativas
-- (fallback) ganha o telefone que enviou.

ALTER TABLE public.bot_chats ADD COLUMN IF NOT EXISTS total_admins integer NOT NULL DEFAULT 0;
ALTER TABLE public.bot_chats ADD COLUMN IF NOT EXISTS total_participantes integer NOT NULL DEFAULT 0;
  ...
  RETURN jsonb_build_object(
    ...
    'chats', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'sessao_telefone', max(s.telefone), 'visto_em', max(c.visto_em),
        'total_admins', max(c.total_admins), 'total_participantes', max(c.total_participantes)
      ) ORDER BY c.nome)
