-- Restore the reviewed legacy-identity scope after the laudo lifecycle
-- migration replaced identity-aware filters with direct UID comparisons.
  ...
  IF internal_request THEN
    INSERT INTO public.internal_access_events (
      ...
      jsonb_build_object(
        ...
        'source_agent_uid', inspection."agenteUid",
...
REVOKE ALL ON FUNCTION public.authorize_inspection_laudo_generation(uuid, text)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.authorize_inspection_laudo_generation(uuid, text)
  TO authenticated;
REVOKE ALL ON FUNCTION public.authorize_internal_customer_document(text, uuid, text)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.authorize_internal_customer_document(text, uuid, text)
  TO authenticated;
REVOKE ALL ON FUNCTION public.get_internal_customer_operations(text)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_internal_customer_operations(text)
  TO authenticated;

COMMENT ON FUNCTION public.get_internal_customer_operations(text) IS
  'Returns customer operations and laudo lifecycle entries using reviewed canonical and legacy agent identities.';
