-- A campaign must be honest about when it will be delivered. The console
-- button said "Agendar envio" while the pipeline delivered immediately.
-- scheduled_at makes the promise real: the RPC validates the window, the row
-- stays queued and the dispatcher only claims campaigns whose schedule is
-- due (the */5 safety-net cron makes sure they are picked up on time).

ALTER TABLE public.notification_campaigns
  ADD COLUMN IF NOT EXISTS scheduled_at timestamptz;

CREATE OR REPLACE FUNCTION public.create_notification_campaign(p_title text, p_body text, p_category text, p_priority text, p_municipio text, p_platforms text[], p_roles text[], p_payload jsonb, p_reason text, p_operation_id uuid, p_scheduled_at timestamptz DEFAULT null)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path = ''
...
BEGIN
  ...
  IF p_scheduled_at IS NOT NULL AND (p_scheduled_at < now() - interval '5 minutes' OR p_scheduled_at > now() + interval '90 days') THEN RAISE EXCEPTION 'invalid_notification_schedule' USING ERRCODE = '22023'; END IF;
  ...
  v_hash := md5(concat_ws('|', v_title, v_body, p_category, p_priority, coalesce(v_municipio, ''), array_to_string(v_platforms, ','), array_to_string(v_roles, ','), coalesce(p_payload, '{}'::jsonb)::text, btrim(p_reason), coalesce(p_scheduled_at, 'immediate')));
  ...
  INSERT INTO public.notification_campaigns(id, title, body, category, priority, municipio, target_platforms, target_roles, payload, created_by, scheduled_at)
  VALUES (v_campaign_id, v_title, v_body, p_category, p_priority, v_municipio, v_platforms, v_roles, private.sanitize_internal_metadata(coalesce(p_payload, '{}'::jsonb)), v_actor, p_scheduled_at);
  ...
  END IF;
  v_previous := jsonb_build_object('ok', true, 'campaign_id', v_campaign_id, 'recipient_count', v_recipient_count, 'status', v_status, 'scheduled_at', p_scheduled_at);
  ...
  VALUES (v_actor, private.current_internal_role(v_actor), 'notification.campaign.create', 'notification_campaign', v_campaign_id::text, 'allowed', left(btrim(p_reason), 500), jsonb_build_object('municipio', v_municipio, 'platforms', v_platforms, 'roles', v_roles, 'recipient_count', v_recipient_count, 'status', v_status, 'scheduled_at', p_scheduled_at));
  RETURN v_previous;
END;
$function$;

REVOKE ALL ON FUNCTION public.create_notification_campaign(text, text, text, text, text, text[], text[], jsonb, text, uuid, timestamptz) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.create_notification_campaign(text, text, text, text, text, text[], text[], jsonb, text, uuid, timestamptz) TO authenticated;

DROP FUNCTION IF EXISTS public.list_notification_campaigns();

CREATE OR REPLACE FUNCTION public.list_notification_campaigns()
 RETURNS TABLE(id uuid, title text, category text, priority text, municipio text, target_platforms text[], target_roles text[], status text, recipient_count bigint, sent_count bigint, failed_count bigint, skipped_count bigint, failure_reason text, created_at timestamp with time zone, completed_at timestamp with time zone, scheduled_at timestamp with time zone)
 LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT campaign.id, campaign.title, campaign.category, campaign.priority, campaign.municipio, campaign.target_platforms, campaign.target_roles, campaign.status,
    count(recipient.id), count(recipient.id) FILTER (WHERE recipient.status = 'sent'), count(recipient.id) FILTER (WHERE recipient.status = 'failed'), count(recipient.id) FILTER (WHERE recipient.status = 'skipped'), campaign.failure_reason, campaign.created_at, campaign.completed_at, campaign.scheduled_at
